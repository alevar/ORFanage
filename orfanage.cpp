...
enum FindMode {
  ...
  LONGEST_MATCH, // default (highest number of inframe bases shared above
                 // thresholds) - if alignment specified - will be decided by
                 // alignment instead;
...
std::string mode_to_string(FindMode mode) {
  switch (mode) {
  case FindMode::START_MATCH:
    return "START_MATCH";
  case FindMode::ALL:
    return "ALL";
  case FindMode::LONGEST:
    return "LONGEST";
  case FindMode::LONGEST_MATCH:
...
struct GlobalParameters {
  ...
  // Mode priority order
  std::vector<FindMode> mode_array{FindMode::LONGEST_MATCH, FindMode::BEST,
                                   FindMode::START_MATCH,   FindMode::LONGEST,
                                   FindMode::FIRST,         FindMode::ALL};
  ...
  for (auto &m : global_params.mode_array) {
    ...
    case START_MATCH:
      if (lhs.start_match != rhs.start_match) {
        return lhs.start_match <
               rhs.start_match; // return true if left start does not match
      }
      break;
    ...
    case FIRST:
      if (lhs.query_start != rhs.query_start) {
        return lhs.query_start >
               rhs.query_start; // here the gt is used since smaller query start
                                // is better (more upstream)
      }
      break;
    default:
...
bool score_gt(const Score &lhs, const Score &rhs) {
  ...
  for (auto &m : global_params.mode_array) {
    ...
    case START_MATCH:
      if (lhs.start_match != rhs.start_match) {
        return lhs.start_match >
               rhs.start_match; // return true if left start does match
      }
      break;
    case ALL: // skip - only relevant later
      break;
    ...
    case FIRST:
      if (lhs.query_start != rhs.query_start) {
        return lhs.query_start <
               rhs.query_start; // here the lt is used since smaller query start
                                // is better (more upstream)
      }
      break;
    default:
...
// groups duplicate ORFs and selects representative template (maximizing score
// within each group) returns map of start/end coordinates as keys to a vector
// of query/template pairs with scores
std::map<std::pair<int, int>,
         std::vector<std::tuple<SEGTP, TX, TX, Score, std::string>>>
flatten(std::vector<std::vector<std::tuple<SEGTP, TX, TX, Score, std::string>>>
            &stats) {
  std::map<std::pair<int, int>,
           std::vector<std::tuple<SEGTP, TX, TX, Score, std::string>>>
      flat;
  std::pair<
      std::map<
          std::pair<int, int>,
          std::vector<std::tuple<SEGTP, TX, TX, Score, std::string>>>::iterator,
      bool>
      fit;
  for (auto &grp : stats) {
    for (auto &sub : grp) {
      ...
      fit = flat.insert(std::make_pair(
          key, std::vector<std::tuple<SEGTP, TX, TX, Score, std::string>>{}));
  ...
  for (auto &kv : flat) {
    std::sort(kv.second.begin(), kv.second.end(),
              [](const std::tuple<SEGTP, TX, TX, Score, std::string> &lhs,
                 const std::tuple<SEGTP, TX, TX, Score, std::string> &rhs) {
...
std::pair<int, int> find_best_stat(
    std::map<std::pair<int, int>,
             std::vector<std::tuple<SEGTP, TX, TX, Score, std::string>>>
        &stats_flat) {
  if (stats_flat.empty()) {
...
int run() {
  ...
  if (global_params.clean_templ ||
      !global_params.reference_fasta_fname
           .empty()) { // under these conditions we need to make sure that the
                       // chains are len%3==0
    transcriptome.correct_chain_len();
  }

  if (global_params.clean_templ) {
    run_stats.num_too_short =
        transcriptome.clean_short_orfs(global_params.cds_minlen);
  }
  ...
  if (global_params.clean_templ) {
    run_stats.num_dirty = transcriptome.clean_cds(
        global_params.rescue, global_params.overhang,
        global_params.spliced_overhang,
        global_params.use_id); // TODO: if we load sequence data into bundles -
  ...
  run_stats.template_duplicates =
      transcriptome.deduplicate(global_params.use_id);
...
#ifdef DEBUG
        if (std::strcmp(q->get_tid().c_str(), "CHS.11626.7") ==
            0) { // rna-XM_011520617.2
...
#pragma omp critical
...
#endif
        {
          if ((global_params.keep_all_cds ||
               global_params.keep_cds_if_not_found) &&
              q->has_cds()) {
            q->build_cds();
          }
    ...
    std::vector<std::vector<std::tuple<SEGTP, TX, TX, Score, std::string>>>
        stats; // segment,query,template,score,notes
    CHAIN segments;
    ...
    for (int qi = 0; qi < bundle_it->size(); qi++) {
      ...
      if (global_params.keep_cds_if_not_found &&
          q->has_cds()) { // if we are keeping CDS if not found - we need to
                          // build it here and store in the original query
                          // transcript
        og_q.build_cds();
      }
...
#pragma omp critical
      ...
      for (int ti = 0; ti < bundle_it->size(); ti++) {
        ...
        if (q->get_start() > t->get_cds_end() ||
            q->get_end() < t->get_cds_start())
          continue;

        int intlen =
            q->exon_chain()->intersection(*t->cds_chain(), segments, true);
        stats.push_back(
            std::vector<std::tuple<SEGTP, TX, TX, Score, std::string>>{});
        if (intlen == 0) {
          stats.back().push_back(std::make_tuple(SEGTP(), *q, *t, Score(),
                                                 "no-overlap")); // empty score
        ...
        // now we need to reconstruct a transcript within the query exon chain
        // for each segment of the template ORF
        for (auto &s : segments) {
          stats.back().push_back(std::make_tuple(s, *q, *t, Score(), "-"));
          // TODO: does it work without sequence avaialble?

#ifdef DEBUG
          if (std::strcmp(q->get_tid().c_str(), "CHS.34256.1") ==
              0) { // rna-XM_011520617.2
            std::cout << "found" << std::endl;
          }
#endif

          int seg_phase = t->cds_chain()->get_phase(
              t->get_strand() == '+' ? s.get_start() : s.get_end(),
              t->get_strand());
#ifdef DEBUG
          ...
          if (!global_params.reference_fasta_fname.empty()) {
            ...
            int ret =
                qseg.rescue_cds(global_params.non_aug, global_params.overhang,
                                global_params.spliced_overhang, t);
            std::get<1>(stats.back().back()) = qseg;
            if (!ret) {
            ...
            if (qseg.get_aa().back() != '.' ||
                ...
                (global_params.non_aug && qseg.get_aa().front() != 'M' &&
                 !(qseg.get_strand() == '+'
                       ? qseg.get_cds_start() == t->get_cds_start()
                       : qseg.get_cds_end() == t->get_cds_end()))) {
              std::get<3>(stats.back().back()).pass = false;
              if (qseg.get_aa().back() != '.') {
...
#ifdef DEBUG
          assert(qseg.get_aa().find(".") ==
                 qseg.get_aa().size() -
                     1); // assert that no premature stop-codons are found
#endif
      ...
      std::map<std::pair<int, int>,
               std::vector<std::tuple<SEGTP, TX, TX, Score, std::string>>>
          stats_flat = flatten(stats); // key: cds start/end; value: segment,
                                       // query, templates, score, notes
      ...
      if (stats_flat.empty() || best_se.first < 0) {
        ...
        if (global_params.mode_array[0] == ALL) { // output all
          ...
          for (auto &kv : stats_flat) {
            ...
            std::string new_tid =
                og_tid + "." + std::to_string(kv.first.first) + "-" +
                std::to_string(
                    kv.first.second); // new name shall consist of the
                                      // tid+start/end cds coordinates
            first_kv.set_tid(new_tid);

            auto &sc = std::get<3>(kv.second.front());
            first_kv.add_attribute("orfanage_status", "1");
            first_kv.add_attribute("orfanage_template_source",
                                   first_kv.get_template()->get_source());
            first_kv.add_attribute("orfanage_template",
                                   first_kv.get_template()->get_tid());
            first_kv.add_attribute("orfanage_duplicity",
                                   std::to_string(first_kv.num_dups()));
            first_kv.add_attribute("orfanage_ilpi", std::to_string(sc.ilpi));
            first_kv.add_attribute("orfanage_lpi", std::to_string(sc.lpi));
            first_kv.add_attribute("orfanage_mlpi", std::to_string(sc.mlpi));
            first_kv.add_attribute("orfanage_start_match",
                                   sc.start_match ? "1" : "0");

            bool stop_match =
                (first_kv.get_strand() == '+'
                     ? (first_kv.get_cds_end() ==
                        first_kv.get_template()->get_cds_end())
                     : (first_kv.get_cds_start() ==
                        first_kv.get_template()->get_cds_start()));
            first_kv.add_attribute("orfanage_stop_match",
                                   stop_match ? "1" : "0");
          ...
          if (sfit->second.size() > 1) {
            for (auto v = sfit->second.begin() + 1; v != sfit->second.end();
                 v++) {
              frontq.merge(std::get<2>(*v)); // merge into best
            }
          }

          auto &sc = std::get<3>(sfit->second.front());
          frontq.add_attribute("orfanage_status", "1");
          frontq.add_attribute("orfanage_template_source",
                               frontq.get_template()->get_source());
          frontq.add_attribute("orfanage_template",
                               frontq.get_template()->get_tid());
          frontq.add_attribute("orfanage_duplicity",
                               std::to_string(frontq.num_dups()));
          frontq.add_attribute("orfanage_ilpi", std::to_string(sc.ilpi));
          frontq.add_attribute("orfanage_lpi", std::to_string(sc.lpi));
          frontq.add_attribute("orfanage_mlpi", std::to_string(sc.mlpi));
          frontq.add_attribute("orfanage_start_match",
                               sc.start_match ? "1" : "0");

          bool stop_match = (frontq.get_strand() == '+'
                                 ? (frontq.get_cds_end() ==
                                    frontq.get_template()->get_cds_end())
                                 : (frontq.get_cds_start() ==
                                    frontq.get_template()->get_cds_start()));
          frontq.add_attribute("orfanage_stop_match", stop_match ? "1" : "0");
        ...
        for (auto &kv : stats_flat) {
          ...
          if (global_params.mode_array[0] == ALL) {
            tid = tid + "." + std::to_string(kv.first.first) + "-" +
                  std::to_string(
                      kv.first.second); // new name shall consist of the
                                        // tid+start/end cds coordinates
          }
          ...
          for (auto &v : kv.second) { // for every segment
            global_params.stats_fp
                << tid << "\t"                      // query
                << std::get<2>(v).get_tid() << "\t" // template
                << std::get<0>(v) << "\t"           // segment
                << std::get<4>(v) << "\t"           // notes
                << std::get<1>(v).num_dups() + 1
                << "\t" // number of duplciates represented by the current ORF
...
// TODO:
//   - ILPI currently divides by the length of the template. Incorrect. Suppose
//   query is 10bp and reference is 5.
//     and suppose all of query matches the reference - this will return 100%
//     ilpi which is incorrect. Instead should be divided by matches+mismatches
//     = 10

int main(int argc, char **argv) {
  ArgParse args("orfanage", "Annotating Open Reading Frames based on "
                            "reference, phylogeny and sequence similarity.");

  args.add_option("query", ArgParse::Type::STRING,
                  "Path to a GTF query file with transcripts to which CDSs are "
                  "to be ported",
                  ArgParse::Level::GENERAL, true);
  args.add_option("output", ArgParse::Type::STRING,
                  "Basename for all output files generated by this software",
                  ArgParse::Level::GENERAL, true);
  args.add_option("reference", ArgParse::Type::STRING,
                  "Path to the reference genome file in FASTA format. This "
                  "parameter is required when the following parameters are "
                  "used: 1. cleanq; 2. cleant; 3. pi.",
                  ArgParse::Level::GENERAL, false);
  args.add_option("cleanq", ArgParse::Type::FLAG,
                  "If enabled - will ensure all transcripts in the output file "
                  "will have a valid start and end codons. This option "
                  "requires the use of --reference parameter",
                  ArgParse::Level::GENERAL, false);
  args.add_option(
      "cleant", ArgParse::Type::FLAG,
      "If enabled - will ensure all ORFs in the reference annotations start "
      "with a valid start codon and end with the first available stop codon. "
      "This option requires the use of --reference parameter",
      ArgParse::Level::GENERAL, false);
  args.add_option(
      "rescue", ArgParse::Type::FLAG,
      "If enabled - will attempt rescuing the broken ORFs in the reference "
      "annotations. This option requires the use of --reference parameter",
      ArgParse::Level::GENERAL, false);
  args.add_option(
      "lpi", ArgParse::Type::INT,
      "Percent identity by length between the original and reference "
      "transcripts. If -1 (default) is set - the check will not be performed.",
      ArgParse::Level::GENERAL, false);
  args.add_option(
      "ilpi", ArgParse::Type::INT,
      "Percent identity by length of bases in frame of the reference "
      "transcript. If -1 (default) is set - the check will not be performed.",
      ArgParse::Level::GENERAL, false);
  args.add_option(
      "mlpi", ArgParse::Type::INT,
      "Percent identity by length of bases that are in both query and "
      "reference. If -1 (default) is set - the check will not be performed.",
      ArgParse::Level::GENERAL, false);
  args.add_option(
      "minlen", ArgParse::Type::INT,
      "Minimum length of an open reading frame to consider for the analysis",
      ArgParse::Level::GENERAL, false);

  args.add_option(
      "mode", ArgParse::Type::STRING,
      "Strategy to select the CDS for transcripts: ALL, LONGEST, "
      "LONGEST_MATCH, FIRST, BEST, START_MATCH. A cascading array of modes can "
      "be provided as a comma-separated list to resolve ties or issues in CDS "
      "selection (eg. two candidate ORFs have the longest length, mode "
      "selection falls back from LONGEST to BEST. Or START_MATCH fails to "
      "match and falls back to FIRST or BEST). Default: " +
          modeArray_to_string(global_params.mode_array) + ".",
      ArgParse::Level::GENERAL, false);

  args.add_option(
      "stats", ArgParse::Type::STRING,
      "Output a separate file with stats for each query/template pair",
      ArgParse::Level::GENERAL, false);
  args.add_option("threads", ArgParse::Type::INT,
                  "Number of threads to run in parallel",
                  ArgParse::Level::GENERAL, false);
  args.add_option("use_id", ArgParse::Type::FLAG,
                  "If enabled, only transcripts with the same gene ID from the "
                  "query file will be used to form a bundle. In this mode the "
                  "same template transcript may be used in several bundles, if "
                  "overlaps transcripts with different gene_ids.",
                  ArgParse::Level::GENERAL, false);
  args.add_option(
      "non_aug", ArgParse::Type::FLAG,
      "If enabled, non-AUG start codons in reference transcripts will not be "
      "discarded and will be considered in overlapping query transcripts on "
      "equal grounds with the AUG start codon.",
      ArgParse::Level::GENERAL, false);

  args.add_option(
      "keep_all_cds", ArgParse::Type::FLAG,
      "Mutually exclusive with '--keep_cds_if_not_found'. If enabled, any CDS "
      "already present in the query will be kept unmodified.",
      ArgParse::Level::GENERAL, false);
  args.add_option(
      "keep_cds_if_not_found", ArgParse::Type::FLAG,
      "Mutually exclusive with '--keep_all_cds'. If enabled, will still search "
      "for new ORF in each query transcript. If query transcript has CDS "
      "annotated, and no ORF can be identified by the method, the original "
      "will be kept. Original CDS will be replaced if a valid ORF can be "
      "found. Use '--keep_all_cds' to retain all unmodified CDS in query.",
      ArgParse::Level::GENERAL, false);

  args.add_option("overhang", ArgParse::Type::INT,
                  "If enabled, will also evaluate nucleotide sequence up and "
                  "downstream up to N bases as set for the argument.",
                  ArgParse::Level::GENERAL, false);
  args.add_option(
      "spliced_overhang", ArgParse::Type::FLAG,
      "Only in effect when combined with the '--overhang' parameter. If "
      ...
      "detected.",
      ArgParse::GENERAL, false);

  args.add_positional_argument(
      "templates", ArgParse::Type::STRING,
      "One or more GFF/GTF files with coding exons to be used as templates.",
      true, true);

  args.add_option("help", ArgParse::Type::FLAG, "Prints this help message.",
                  ArgParse::Level::HELP, false);
  ...
  try {
    ...
    if (args.is_set("reference")) { // much from gpertea bamcons.cpp
      ...
      GFastaIndex faIdx(args.get_string("reference").c_str(),
                        fa_idx_fname.c_str());
      if (!faIdx.hasIndex()) {
        std::cerr << "No fasta index found for " << fa_idx_fname
                  << ". Building now" << std::endl;
        faIdx.buildIndex();
        if (faIdx.getCount() == 0) {
          throw std::runtime_error(
              "No fasta records found in the reference file: " +
              global_params.reference_fasta_fname);
        }
        FILE *fcreate = fopen(fa_idx_fname.c_str(), "w");
        if (fcreate == nullptr) {
          throw std::runtime_error("Cannot create fasta index: " +
                                   fa_idx_fname);
        }
        if (faIdx.storeIndex(fcreate) < faIdx.getCount()) {
          throw std::runtime_error("Error writing the fasta index file: " +
                                   fa_idx_fname);
        }
    ...
    if (args.is_set("mode")) {
      ...
      while (std::getline(ss, mode, ',')) {
        ...
        mode.erase(mode.begin(),
                   std::find_if(mode.begin(), mode.end(), [](unsigned char ch) {
                     return !std::isspace(ch);
                   }));
        mode.erase(
            std::find_if(mode.rbegin(), mode.rend(),
    ...
    if (global_params.clean_templ || global_params.clean_query ||
        global_params.rescue) {
      assert(args.is_set("reference"));
    }

    global_params.cds_minlen = args.is_set("minlen")
                                   ? args.get_int("minlen")
                                   : default_params.cds_minlen;
    global_params.len_perc_ident = args.is_set("lpi")
                                       ? args.get_int("lpi")
                                       : default_params.len_perc_ident;
    global_params.len_frame_perc_ident =
        args.is_set("ilpi") ? args.get_int("ilpi")
                            : default_params.len_frame_perc_ident;
    global_params.len_match_perc_ident =
        args.is_set("mlpi") ? args.get_int("mlpi")
                            : default_params.len_match_perc_ident;

    global_params.overhang = args.is_set("overhang") ? args.get_int("overhang")
                                                     : default_params.overhang;
    global_params.spliced_overhang =
        args.is_set("spliced_overhang") ? true : false;

    global_params.num_threads = args.is_set("threads")
                                    ? args.get_int("threads")
                                    : default_params.num_threads;
#ifndef DEBUG
#ifdef OPENMP_AVAILABLE
    ...
    if (global_params.keep_all_cds && global_params.keep_cds_if_not_found) {
      throw std::runtime_error("Mutually exclusive flags: '--keep_all_cds' and "
