...
TX::TX(uint seqid, GffObj *tx, int idx, bool is_templ) {
  if (tx->hasCDS()) {
    ...
    case '.':
      ...
      break;
    default:
      throw std::runtime_error("Unknown CDS phase for the initial segment");
    }
  }
  ...
  for (int i = 0; i < tx->exons.Count(); i++) {
    this->exons.push_back(
        SEGTP((int)tx->exons.Get(i)->start, (int)tx->exons.Get(i)->end, 0));
  }
  ...
  if (tx->attrs != nullptr) {
    ...
    for (int i = 0; i < tx->attrs->Count(); i++) {
      const char *attrname =
          tx->names->attrs.getName(tx->attrs->Get(i)->attr_id);
      const char *attrval = tx->attrs->Get(i)->attr_val;
      if (attrval == nullptr || attrval[0] == '\0')
        continue;
      ...
      if (Gstrcmp(attrname, "geneID") == 0 ||
          strcmp(attrname, "gene_id") == 0) {
        continue;
      }
      ait = this->attrs.insert(
          std::make_pair(attrname, tx->attrs->Get(i)->attr_val));
      if (!ait.second) {
        throw std::runtime_error("Attribute: " + std::string(attrname) +
                                 " already exists");
      }
    }
  }
...
void TX::build_cds() {
  ...
  if (this->cds_phase != 0) {
    if (this->strand == '+') {
      this->cds_start = this->exons.nt2genome(
          this->exons.genome2nt(this->cds_start, this->strand) + cds_phase,
          this->strand);
    } else {
      this->cds_end = this->exons.nt2genome(
          this->exons.genome2nt(this->cds_end, this->strand) + cds_phase,
          this->strand);
    }
    this->cds_phase = 0;
  }
...
bool TX::add_attribute(const std::string& k, const std::string& v) {
  std::pair<std::map<std::string, std::string>::iterator, bool> ait;
...
void TX::set_bundle_ref(Bundle *b) { this->bundle = b; }
void TX::_get_nt_seq(CHAIN &chain, std::string &res) {
  ...
  if (this->strand == '+') {
    ...
    for (int ci = chain.size() - 1; ci >= 0; ci--) {
      std::string tmp =
          this->bundle->get_nts(chain[ci].get_start(), chain[ci].get_end());
...
// TODO: extension - should we create a wrapper class for sequence data such
// that handles extension? What if extension is enabled for the bundle (bundle
// load extended data), but this function forgets the argument?
//    bundle should load sequence with extension and adjust start coordinate.
//    when transcripts load sequence, they should store extension separately,
//    thus only querying it if necessary, but otherwise not going over bound if
//    extension is necessary and transcript coordinates are adjusted - the part
//    of the extension from the sequence needs to be poped from extension and
//    merged into main transcript sequence store two extensions - 3' and 5'
int TX::adjust_stop() { // adjust the chain coordinates to stop at the first
                        // stop codon
  if (this->seq.empty()) {
    return 0;
  }
...
int TX::adjust_start() { // adjust the chain coordinates to start at the first
                         // start codon
  if (this->seq.empty()) {
    return 0;
  }
...
int TX::positions(
    int pos, uint num_positions, std::string &nc,
    bool forward) { // return the nts starting at position - these are returned
                    // with respect to the + strand and will need to be reverse
                    // complemented separately
  nc = std::string(num_positions, '-');
  int added = 0;
  int np = pos;
  const auto &segs = this->exons.get_segments();
  ...
  for (int i = start_i; i != end_i; i += step) {
    int es = segs[i].get_start(), ee = segs[i].get_end();
    if (forward && np > ee)
      continue;
    if (!forward && np < es)
      continue;

    int start = forward ? std::max(np, es) : std::min(np, ee);

    while ((forward ? start <= ee : start >= es) && added < num_positions) {
      int res_idx = forward ? added : (num_positions - 1) - added;
      nc[res_idx] = this->bundle->get_nt(start);
      np = start;
      added++;
      start += step;
    }
    if (added == num_positions)
      break;
    np = forward ? ee + 1 : es - 1;
  }
...
int TX::next_positions(
    int last_position, uint num_positions, std::string &nc,
    bool forward) { // return the nts of the next positions - these are returned
                    // with respect to the + strand and will need to be reverse
                    // complemented separately
  int np = -1;
  const auto &segs = this->exons.get_segments();
  ...
  for (int i = start_i; i != end_i; i += step) {
    int es = segs[i].get_start(), ee = segs[i].get_end();
    if ((forward && last_position < es) || (!forward && last_position > ee)) {
      np = forward ? es : ee;
      break;
    }
    if (last_position >= es && last_position <= ee) {
      if (forward && last_position < ee) {
        np = last_position + 1;
        break;
      } else if (!forward && last_position > es) {
        np = last_position - 1;
        break;
      }
    }
...
char TX::get_codon_aa(uint pos) {
  ...
  } else { // we did not pre-load the sequence and need to specifically load
           // just the codon
    pos *= 3;
...
// when extending sequence - check for AG/GT (depending on the direction and
// terminate
void TX::extend_seq(int extend_len, bool spliced_extend, bool towards_stop) {
  ...
  if (extend_len > 0) { // consider additional sequence
    ...
    if ((this->strand == '+' && towards_stop) ||
        (this->strand == '-' && !towards_stop)) {
      extension_start =
          std::min(this->exons.get_end() + 1, (int)this->bundle->get_end());
      extension_end = std::min(this->exons.get_end() + extend_len,
                               (int)this->bundle->get_end());
    } else { // strand=='-'
      extension_start = std::max(this->exons.get_start() - extend_len,
                                 (int)this->bundle->get_start());
      extension_end =
          std::max(this->exons.get_start() - 1, (int)this->bundle->get_start());
    }
    if (this->strand == '+') {
    ...
    if (towards_stop) {
      if (spliced_extend) { // if requested to consider donor/acceptor and since
                            // extending to stop - search for GT and if found -
                            // trim to there to avoid extending past the
                            // potential junction
        // trim extension_str to before the first GT
        transform(extension_str.begin(), extension_str.end(),
                  extension_str.begin(), ::toupper);
    ...
    } else { // if extending the start - need to also adjust cds_start and
             // cds_end accordingly
      if (spliced_extend) { // if requested to consider donor/acceptor and since
                            // extending to start - search for AG and if found -
                            // trim to there to avoid extending past the
                            // potential junction
...
void TX::extend_to_stop(
    int extend_len,
    bool spliced_extend) { // searches downstream of the CDS for the next stop
                           // codon in the same frame
  if (this->seq.get_aa(this->seq.cds_aa_len() - 1) ==
  ...
  uint res =
      this->seq.find_inframe_codon('.', ';', stops, this->seq.get_cds_end() + 1,
                                   this->seq.get_exon_len(), true, true);
  if (res != 0) { // stop is found - adjust accordingly
    // if extension requested and stop found within extension - set new
    // transcript coordinates
    if (extend_len > 0) {
      int extension_len = ((stops[0] + 2) - this->exons.clen()) + 1;
      extension_len = std::max(
          extension_len, 0); // if extension is less than 0 - means stop found
                             // within transcript and not in the extension. Need
                             // to trim to the original coordinates
      if (strand == '+') {
...
// this version will search for the first available start codon resulting in the
// longest possible ORF
void TX::extend_to_start(
    int new_start, int extend_len,
    bool spliced_extend) { // searches upstream for the available start codons
  if (new_start == -1) {   // start coordinate is not provided - search
    ...
    uint res = this->seq.find_inframe_codon(
        'M', '.', starts, this->seq.get_cds_start(), 0, false, true);

    if (res != 0) { // start is found - adjust accordingly
      // if extension requested and start found within extension - set new
      // transcript coordinates
      if (extend_len > 0) {
        int extension_len = extend_len - starts[0];
        extension_len = std::max(
            extension_len, 0); // if extension is less than 0 - means stop found
                               // within transcript and not in the extension.
                               // Need to trim to the original coordinates
        if (strand == '+') {
          this->exons.begin()->set_start(this->exons.get_start() -
                                         extension_len);
        } else {
          (this->exons.end() - 1)
              ->set_end(this->exons.get_end() + extension_len);
        }
      }
      new_start = starts[0];
    }
...
// this version of the extension function will search for the start codon which
// maximizes the similarity to the original template ORF
bool TX::extend_to_start(
    TX *ref_tx, bool allow_non_aug) { // extends to start while comparing to the
                                      // closest reference
  // can skip the ckeck if the start coordinate is the same as the reference and
  // is M
  char start_codon = this->get_codon_aa(0);
  bool match_start = this->strand == '+'
                         ? this->get_cds_start() == ref_tx->get_cds_start()
                         : this->get_cds_end() == ref_tx->get_cds_end();
  if ((start_codon == 'M' || allow_non_aug) && match_start) {
    return true;
  }

  // check non-aug
  if (allow_non_aug) {
    // check if reference start matching is possible (much faster) - if yes -
    // use that, otherwise proceed evaluating other possibilities
    int ref_cds_start =
        this->strand == '+' ? ref_tx->get_cds_start() : ref_tx->get_cds_end();
    int this_cds_start =
        this->strand == '+' ? this->get_cds_start() : this->get_cds_end();
    ...
    if (ref_start_in_this >= 0 &&
        ref_start_in_this % 3 ==
            q_start_in_this % 3) { // found start - can extend and exit
      // need to make sure no stop codons are being introduced in the process
      std::vector<uint> stops;
      uint res = this->seq.find_inframe_codon('.', ';', stops,
                                              this->seq.get_cds_start(),
                                              ref_start_in_this, false, true);
      if (res == 0) {
  ...
  uint res =
      this->seq.find_inframe_codon('M', '.', transcriptomic_starts,
                                   this->seq.get_cds_start(), 0, false, false);
  if (res == 0) {
    return false;
  }

  int max_chain_extension_len =
      this->seq.get_cds_start() - transcriptomic_starts.back();
  ...
  // 3. else compute intersection and find the last start codon which adds
  // inframe matches == maximizes similarity compute intersection (q and t only)
  ...
  auto get_best_start = [&best_start_idx, &transcriptomic_starts, &ref_tx,
                         this](SEGTP &c) {
    // as soon as we find the first valid (with added inframe) - save and
    // terminate transcriptomic positions with respect to q and t
    uint t_tpos =
        ref_tx->cds.genome2nt(c.get_start(this->strand), this->strand);
  ...
  if (this->strand == '+') {
    (void)std::find_if(intersected_chain.begin(), intersected_chain.end(),
                       get_best_start);
  } else {
    (void)std::find_if(intersected_chain.rbegin(), intersected_chain.rend(),
                       get_best_start);
  }
  ...
  // 4. cut to length
  int start_nt_pos =
      transcriptomic_starts[best_start_idx] - transcriptomic_starts.back();
...
uint TX::inframe_len(
    TX *t) { // it doesn't matter that we do this stranded - either way the
             // number of inframe codons should be the same
  uint res = 0;
...
int TX::rescue_cds(bool allow_non_aug, int extend_len, bool spliced_extend,
                   TX *t) {
  if (this->seq.cds_nt_len() < 3) {
    this->remove_cds();
    return 0;
  ...
  } else { // try to find a start codon guided by reference. If not - search
           // normally
  ...
  if (allow_non_aug && t != nullptr) {
    run_adjust_start =
        !(this->strand == '+' ? this->cds_start == t->get_cds_start()
                              : this->cds_end == t->get_cds_end());
  }
  if (run_adjust_start) {
    adjust_start();
  }
...
void TX::remove_seq() { this->seq.clear(); }
void TX::load_seq() {
  this->seq.clear();
  std::string nt_seq;
...
void TX::compare(TX &t, CHAIN &res) { return this->cds.compare(t.cds, res); }
Score TX::score(TX &t) {
  Score s;
  s.qlen = this->cds_len();
  s.tlen = t.cds_len();
  ...
  // 1. compute the total number of matching positions between query and
  // template
  // 2. compute the number of matching positions in frame between query and
  // template
  CHAIN mod_chain;
  ...
  auto extract_mods = [&s, &t_frame, &q_frame](SEGTP &mc) {
    if (mc.get_phase() == -1) { // extra positions in the query
      ...
      q_frame += mc.slen();
    } else if (mc.get_phase() ==
               1) { // template positions missing from the query
      s.num_bp_missing += mc.slen();
      t_frame += mc.slen();
    } else if (mc.get_phase() ==
               0) { // matching positions between query and template
      ...
      throw std::runtime_error("Wrong code phase encountered during scoring");
    }
  };
  if (strand == '+') {
  ...
  s.query_start = this->strand == '+'
                      ? this->cds_start - this->exons.get_start()
                      : this->exons.get_end() - this->cds_end;

  s.start_match = this->strand == t.strand &&
                  (this->strand == '+' ? this->cds_start : this->cds_end) ==
                      (this->strand == '+' ? t.cds_start : t.cds_end);
...
void Bundle::load_seq(GFaSeqGet *seq) {
  if ((this->bstart + this->blen() - 1) > seq->getseqlen() ||
      this->bstart < 0) {
    throw std::runtime_error(
        "Bundle + stop codon extends past the end of the reference sequence");
  }
...
void Transcriptome::use_non_aug() { this->allow_non_aug = true; }
void Transcriptome::load_seqids(GffReader &gffReader) {
  for (int i = 0; i < gffReader.gflst.Count(); ++i) {
    ...
    this->n2i_it = this->seqnames_ids.insert(
        std::make_pair(pGffObj->getGSeqName(), this->seqid_names.size()));
...
void Transcriptome::add(const std::string &gtf_fname, bool is_templ,
                        bool coding_only) {
  this->bundles.clear(); // any pointers that previously existed will be
                         // invalidated when new data is added
  FILE *gff_file = fopen(gtf_fname.c_str(), "r");
  if (gff_file == nullptr) {
    throw std::runtime_error("Couldn't open the GTF file: " + gtf_fname);
  }
  ...
  for (int i = 0; i < gffReader.gflst.Count(); ++i) {
    ...
    if (!pGffObj->isTranscript()) { // skip records which could not be
                                    // transformed into a transcript
      continue;
    }
...
uint Transcriptome::bundleup(
    bool use_id, uint32_t overhang,
    bool spliced_extend) { // create bundles and return the total number of
                           // bundles
  // if rescue_len is enabled - will extend bundles by that number of bases to
  // enable rescue lookup up and down stream of the original locus coordinates
  this->sort(use_id);
  ...
  if (this->check_ref) {
    ...
    for (auto &t : this->tx_vec) {
      ...
      if (!current_seq_valid) {
        if (!t.is_template()) {
          throw std::runtime_error(
              "Reference FASTA is missing sequence for query: " + seq_name);
        }
      } else {
  ...
  if (this->check_ref &&
      !this->tx_vec
           .empty()) { // preload reference based on the front transcript
  ...
  for (auto &t : this->tx_vec) {
    if (!this->bundles.back().can_add(&t, use_id)) {
      if (this->check_ref) {
        ...
        uint32_t new_start_pos = (uint32_t)std::max(
            (int)this->bundles.back().get_start() - (int)overhang,
            (int)1); // convert to int to avoid overflow when subtracting
                     // overhang
        uint32_t new_end_pos =
            std::min(this->bundles.back().get_end() + overhang,
                     (uint32_t)seqid_seq->getseqlen());
  ...
  if (this->check_ref) {
    ...
    uint32_t new_start_pos = (uint32_t)std::max(
        (int)this->bundles.back().get_start() - (int)overhang,
        (int)1); // convert to int to avoid overflow when subtracting overhang
    uint32_t new_end_pos = std::min(this->bundles.back().get_end() + overhang,
                                    (uint32_t)seqid_seq->getseqlen());
    // make sure these operations did not shorten the data
    assert(new_start_pos <= this->bundles.back().get_start());
    assert(new_end_pos >= this->bundles.back().get_end());
...
uint Transcriptome::clean_cds(bool rescue, int extend_len, bool spliced_extend,
                              bool use_id) {
  uint res = 0;
  ...
  for (auto &bundle : this->bundles) {
    for (auto &tx : bundle) {
      ...
      if (this->check_ref) {
        if (rescue) {
          tx->rescue_cds(
              this->allow_non_aug, extend_len,
              spliced_extend); // TODO: allow non aug - needs to allow storage
                               // of non-aug transcripts (only perform cleaning
                               // based on stop codons) and no start extension.
                               // And needs to handle querrying appropriately
        }
...
// if requested - comparisons are only made between transcripts with the same
// geneID
...
uint Transcriptome::deduplicate(bool use_id) {
  ...
  if (use_id) {
    auto last = std::unique(this->tx_vec.begin(), this->tx_vec.end(),
                            unique_tx_cmp_with_id);
    this->tx_vec.erase(last, this->tx_vec.end());
  } else {
    auto last =
        std::unique(this->tx_vec.begin(), this->tx_vec.end(), unique_tx_cmp);
    this->tx_vec.erase(last, this->tx_vec.end());
  }
...
void Transcriptome::remove_non_coding() {
  this->tx_vec.erase(std::remove_if(this->tx_vec.begin(), this->tx_vec.end(),
                                    [](const TX &tx) { return !tx.has_cds(); }),
                     this->tx_vec.end());
}